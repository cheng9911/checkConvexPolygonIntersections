clc, clear, close all;
% 从 Excel 文件中读取封闭图形的点集并转置
data = readmatrix('step5.xlsx');
points = data(:, 1:2)'; % 转置点集，使其为 2xN 格式
tic;
% 定义长方形的四个顶点
rectangle = [-40,148 ; 5, 148; 5, 140; -40, 140]';
% 计算最短距离以及最近的两个点
[distance, closest_rect_point, closest_polygon_point] = gjk_rectangle_polygon(rectangle, points);
